...
// Send course access expiry reminder to student
const sendCourseExpiryReminder = async (email, userName, courseTitle, expiryDate) => {
  try {
    const formattedDate = new Date(expiryDate).toLocaleDateString('en-IN', {
      year: 'numeric',
      month: 'long',
    ...
    await transporter.sendMail({
      from: `"AlphaKnowledge" <${process.env.EMAIL_USER}>`,
      to: email,
      subject: `Your access to ${courseTitle} is expiring soon`,
      html: `
        <!DOCTYPE html>
        <html>
        <head>
          <style>
            ...
            .content { background: #f9f9f9; padding: 30px; border-radius: 0 0 10px 10px; }
            .footer { text-align: center; margin-top: 20px; color: #666; font-size: 12px; }
            .btn { display: inline-block; background: #fda085; color: white; padding: 12px 25px; text-decoration: none; border-radius: 5px; margin-top: 20px; font-weight: bold; }
          </style>
        </head>
        <body>
          <div class="container">
            ...
            <div class="content">
              <p>Hello <strong>${userName}</strong>,</p>
              <p>This is a reminder that your access to the course <strong>${courseTitle}</strong> is expiring soon on <strong>${formattedDate}</strong>.</p>
              <p>After this date, you will no longer be able to access the course content or your progress.</p>
              <p>To continue learning without interruption, please visit the course catalog to renew your access.</p>
              <center><a href="${process.env.FRONTEND_URL}/courses" class="btn">View Course Catalog</a></center>
            </div>
            <div class="footer">
              <p>© 2026 AlphaKnowledge. All rights reserved.</p>
            </div>
    ...
    return { success: true, message: 'Course expiry reminder sent' };
  } catch (error) {
    console.error('Error sending course expiry reminder:', error);
