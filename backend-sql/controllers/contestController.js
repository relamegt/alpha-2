...
// Get all contests (Admin/Instructor)
const getAllContests = async (req, res) => {
    try {
        const query = {};
        const contests = await prisma.contest.findMany({ where: query, orderBy: { createdAt: 'desc' } });
...
const createContest = async (req, res) => {
    try {
        const {
            title, description, startTime, endTime, problems, batchId,
            proctoringEnabled, tabSwitchLimit, maxViolations,
            duration, maxAttempts, newProblemsData = []
        } = req.body;
        ...
        const existingProblemIds = Array.isArray(problems) ? problems.filter(p => typeof p === 'string') : [];
        ...
        if (newProblemsData.length > 0) {
            for (const p of newProblemsData) {
                ...
                newProblemIds.push(newProblem.id);
            }

            // 3. Update Contest
            const allProblemIds = [...existingProblemIds, ...newProblemIds];
            await Contest.update(contest.id, {
    ...
    try {
        ...
        if (batchId === 'all') {
            ...
            if (req.user.role === 'instructor') {
                ...
                const assignedBatches = (instructor?.assignedBatches || []).map(b => b.id);
        ...
        const [contests, total] = await Promise.all([
            ...
            prisma.contest.count({ where: query })
        ]);
        ...
        if (req.user.role === 'student') {
            ...
            const contestsWithSubmissionStatus = contests.map(contest => {
                const subForThisContest = submissions.find(s => s.contestId.toString() === contest.id.toString());
                return {
                    ...contest,
                    isSubmitted: !!subForThisContest,
                    isSolved: !!subForThisContest,
                    totalProblems: contest._count?.problems || 0,
        ...
        // For instructors/admins, return with problem count
        res.json({
            success: true,
            count: contests.length,
            total,
            ...
            contests: contests.map(c => ({
                ...c,
                totalProblems: c._count?.problems || 0
            }))
        });
    } catch (error) {
...
// getContestsByCourse removed as it belongs to courseContestController now
...
const getContestById = async (req, res) => {
    try {
        ...
        const effectiveEndTime   = contest.endTime;

        const solvedProblems   = await ContestSubmission.getAcceptedProblems(studentId, contest.id, null);
        const solvedProblemIds = solvedProblems.map(p => p.toString());

        const validProblems = contest.problems.map((problem) => {
            if (!problem) return null;
            const problemId    = problem.id.toString();
            const isProblemSolved  = solvedProblemIds.includes(problemId);

            // Hide test cases and editorial during active contest
            const isContestActive = now >= new Date(effectiveStartTime) && now <= new Date(effectiveEndTime);
        ...
        res.json({
            ...
            contest: {
                ...
                totalProblems:     Math.max(validProblems.length, contest._count?.problems || 0, contest.problems?.length || 0),
                isSubmitted:       hasSubmitted,
                startTime:         effectiveStartTime,
                endTime:           effectiveEndTime,
                currentAttempt:    currentAttempt,
                completedAttempts: contest.completedAttempts || 0,
...
const updateContest = async (req, res) => {
    try {
        const { contestId } = req.params;
        const { title, description, startTime, endTime, problems, batchId, proctoringEnabled, tabSwitchLimit, maxViolations, duration, maxAttempts } = req.body;

        const updateData = {
            title, description, startTime, endTime, problems, batchId, proctoringEnabled, tabSwitchLimit, maxViolations, duration, maxAttempts
        };
...
const submitContestCode = async (req, res) => {
    ...
    try {
        ...
        const isProblemSolved = await ContestSubmission.isProblemSolved(studentId, contest.id, problemId);
        ...
        // Removed isSolo/Solo Mode Logic for regular contests
        let effectiveStartTime = new Date(contest.startTime);
        let effectiveEndTime = new Date(contest.endTime);
        // ------------------------

        const isActive = now >= effectiveStartTime && now <= effectiveEndTime;

        let isGracePeriodActive = false;
        if (isAutoSubmit && now > effectiveEndTime) {
            const graceEndTime = new Date(effectiveEndTime.getTime() + 2 * 60000); // 2 mins grace period
            if (now <= graceEndTime) {
        ...
        if (!isActive && !isGracePeriodActive) {
            return res.status(400).json({
                success: false,
                message: now < effectiveStartTime ? 'Contest has not started yet' : 'Contest has already ended'
            });
        }
        ...
        if (contest.proctoringEnabled && !isAutoSubmit && totalViolations >= contest.maxViolations) {
            notifyViolation(contest.id, studentId, {
    ...
    try {
        ...
        if (isMultiCustom) {
            ...
            const [userResult, solResult] = await Promise.all([
                executeRunner(language, code, userTestCases, problem.timeLimit, problem.id, problem),
                needsSolution
                    ? executeRunner(solLang, solCode, casesNeedingSolution.map(c => ({ input: c.input, output: undefined, isHidden: false })), problem.timeLimit, problem.id, problem).catch(e => {
            ...
            notifyExecutionResult(contest.id, studentId, runResult);
        ...
        if (isCustom) {
            ...
            const [userResult, solRunResult] = await Promise.all([
                executeRunner(language, code, userTestCase, problem.timeLimit, problem.id, problem),
                solCode && solLang
                    ? executeRunner(solLang, solCode, [{ input: customInput, output: undefined, isHidden: false }], problem.timeLimit, problem.id, problem).catch(e => {
                        console.warn('[Contest Run] Solution code failed:', e.message);
                        return null;
                    })
                    : Promise.resolve(null)
...
const finishContest = async (req, res) => {
    ...
    try {
        ...
        // Check if individual contest time is up for Solo mode
        if (contest.isSolo && req.user.role === 'student') {
            const startRecord = await ContestSubmission.getOrCreateStartRecord(studentId, contestId);
            const startTime = new Date(startRecord.submittedAt);
            const durationMs = (contest.duration || 60) * 60 * 1000;
            const endTime = new Date(startTime.getTime() + durationMs);
            const graceEndTime = new Date(endTime.getTime() + 5 * 60 * 1000); // 5 min grace for finish

            if (new Date() > graceEndTime) {
                return res.status(400).json({
                    success: false,
                    message: 'Contest time has expired'
                });
            }
        }
        ...
        const dbViolations = await ContestSubmission.getProctoringViolations(studentId, contest.id);
        ...
        // Find current attempt number for Solo contest
        let attemptNumber = 1;
        if (contest.isSolo) {
            const startRecord = await ContestSubmission.getOrCreateStartRecord(studentId, contestId);
        ...
        await ContestSubmission.markContestCompleted(studentId, contest.id, 0, violationSnapshot, attemptNumber);
...
const getContestLeaderboard = async (req, res) => {
    try {
        ...
        // Reformat problems to consistent interface
        if (contest && contest.problems) {
            contest.problems = contest.problems.map(p => ({ id: p.id, _id: p.id, title: p.title }));
        }
...
const getStudentContestSubmissions = async (req, res) => {
    try {
        ...
        if (contest && contest.isSolo) {
            const startRecord = await ContestSubmission.getOrCreateStartRecord(studentId, contestId);
            attemptNumber = startRecord.attemptNumber || 1;
        }

        const submissions = await ContestSubmission.findByStudentAndContest(studentId, contestId, attemptNumber);
...
const getProctoringViolations = async (req, res) => {
    try {
        ...
        const violations = await ContestSubmission.getProctoringViolations(studentId, contestId, attemptNumber);
            ...
            setTimeout(async () => {
                try {
                    ...
                    if (buffered) {
                        ...
                        // Fetch active attempt specifically for logging
                        const contest = await Contest.findById(contestId);
                        let attemptNumber = 1;
                        if (contest && contest.isSolo) {
                            const startRecord = await ContestSubmission.getOrCreateStartRecord(studentId, contestId);
...
const registerSpotUser = async (req, res) => {
    try {
        ...
        await User.updateSession(user.id, token, 'spot-fingerprint');
...
const unlockContestForUser = async (req, res) => {
    try {
        ...
        const redis = getRedis();
        const unlockLockKey = `lock:unlock:${studentId}:${contest.id}`;
...
const startNewAttempt = async (req, res) => {
    try {
        ...
        if (!contest.isSolo) {
            return res.status(400).json({ success: false, message: 'Only solo contests support multiple attempts' });
        }

        // Count existing COMPLETED records using find().limit() for Astra DB reliability
        const completedDocs = await prisma.contestSubmission.findMany({
            where: {
                studentId: String(studentId),
        ...
        if (contest.maxAttempts && completedCount >= contest.maxAttempts) {
            return res.status(400).json({
                success: false,
                message: `Maximum attempts (${contest.maxAttempts}) reached for this contest`
            });
        }

        const newStart = await ContestSubmission.startNewAttempt(studentId, contestId);

        // Clear all previous violation records for this contest so the student starts fresh
        ...
        res.json({
            ...
            startTime: newStart.submittedAt,
            attemptNumber: newStart.attemptNumber,
            duration: contest.duration
        });
    } catch (error) {
        console.error('Start new attempt error:', error);
        res.status(500).json({ success: false, message: 'Failed to start new attempt', error: error.message });
    }
};

module.exports = {
    getAllContests, // Ensure this is here
    createContest,
    getContestsByBatch,
    getContestById,
    updateContest,
