...
const getBatchAnalytics = async (req, res) => {
    try {
        ...
        // Chunking student IDs for prisma count to avoid potential query size limits
        ...
        for (let i = 0; i < studentIds.length; i += CHUNK_SIZE) {
            ...
            const [chunkTotal, chunkAccepted] = await Promise.all([
                prisma.submission.count({ where: { studentId: { in: chunkIds } } }),
                prisma.submission.count({ where: { studentId: { in: chunkIds }, verdict: 'Accepted' } })
            ]);
...
const getAssignmentAnalytics = async (req, res) => {
    try {
        ...
        // 1. Calculate Heatmap (TestCase ID -> [PassCount, FailCount])
        const heatmap = {};
        testCases.forEach(tc => {
            heatmap[tc.id] = { name: tc.name, passed: 0, failed: 0 };
        });
        ...
        submissions.forEach(sub => {
            if (sub.testResults && Array.isArray(sub.testResults)) {
                sub.testResults.forEach(res => {
                    if (heatmap[res.id]) {
                        if (res.passed) heatmap[res.id].passed++;
                        else heatmap[res.id].failed++;
                    }
                });
            }
        });

        // 2. Common Failures (Top 3 failed test cases)
        const commonFailures = Object.values(heatmap)
        ...
        res.json({
            ...
            analytics: {
                ...
                averageScore: submissions.length > 0 ? (submissions.reduce((acc, s) => acc + s.score, 0) / submissions.length).toFixed(1) : 0,
                heatmap,
                commonFailures
            }
        });
