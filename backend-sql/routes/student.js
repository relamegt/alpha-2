...
router.use((req, res, next) => {
    // Allow profile update, dashboard, and my-courses without profile completion
    if (req.path === '/profile' || req.path === '/dashboard' || req.path === '/my-courses') {
        return next();
    }
...
// Lightweight: assigned courses only (used by navbar)
router.get('/my-courses', profileController.getAssignedCourses);
...
router.post('/courses/:courseId/rate', profileController.rateCourse);
...
router.get('/submission/:problemId/latest', validateObjectId('problemId'), submissionController.getLatestSubmissionByProblem);
