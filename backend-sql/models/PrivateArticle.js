...
async function uniqueSlug(title) {
    ...
    let counter = 2;
    while (counter <= 100) {
        const existing = await prisma.privateArticle.findUnique({ where: { slug: candidate } });
...
class PrivateArticle {
    static async create(data) {
        const slug = await uniqueSlug(data.title);
        return await prisma.privateArticle.create({
            data: {
                slug,
                title: data.title,
    ...
    static async findById(idOrSlug) {
        let article = null;
        try { article = await prisma.privateArticle.findUnique({ where: { id: idOrSlug } }); } catch (e) {}
        if (!article) { article = await prisma.privateArticle.findUnique({ where: { slug: idOrSlug } }); }
        return article;
    }
    ...
    static async findByIds(ids) {
        ...
        return await prisma.privateArticle.findMany({
    ...
    static async update(idOrSlug, updateData) {
        ...
        return await prisma.privateArticle.update({
            where: { id: article.id },
            data: validData
        });
    }
    ...
    static async delete(idOrSlug) {
        ...
        return await prisma.privateArticle.delete({ where: { id: article.id } });
    }

    static async count() {
        return await prisma.privateArticle.count({ where: { isContestProblem: false } });
    }
}

module.exports = PrivateArticle;
