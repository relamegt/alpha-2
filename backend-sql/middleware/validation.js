...
const validateSignup = [
    ...
    body('firstName').trim().notEmpty().withMessage('First name is required'),
    body('lastName').trim().notEmpty().withMessage('Last name is required'),
    body('role').optional().isIn(['student']).withMessage('Only student role is allowed for public signup'),
    handleValidationErrors
];
...
const validateProblemCreation = [
    ...
    body('type').optional().isIn(['problem', 'quiz', 'material', 'video', 'article', 'sql']),
    body('difficulty').optional().isIn(['Easy', 'Medium', 'Hard']).withMessage('Invalid difficulty'),
    body('description').custom((value, { req }) => {
        // Only required for coding problems (type === 'problem' or 'sql' or type is absent/undefined)
        const type = req.body.type;
        const isCodingProblem = !type || type === 'problem' || type === 'sql';
        if (isCodingProblem && (!value || !value.trim())) {
            throw new Error('Description is required for coding problems');
        }
        // For quiz, material, video, article — description is always optional
        return true;
    }),
    body('testCases').custom((value, { req }) => {
        // Only required for coding problems
        const type = req.body.type;
        const isCodingProblem = !type || type === 'problem' || type === 'sql';
        if (isCodingProblem && (!Array.isArray(value) || value.length === 0)) {
            throw new Error('At least one test case is required for coding problems');
        }
        return true;
    }),
    body('videoUrl').custom((value, { req }) => {
        if (req.body.type === 'video' && (!value || !value.trim())) {
            throw new Error('Video URL is required for video lessons');
        }
        return true;
    }),
    body('articleLink').custom((value, { req }) => {
        if (req.body.type === 'article' && (!value || !value.trim())) {
            throw new Error('Article link is required for articles');
        }
        return true;
    }),
    handleValidationErrors
...
const validateSubmission = [
    ...
    body('language').isIn(['c', 'cpp', 'java', 'python', 'javascript', 'csharp', 'mysql', 'postgresql', 'postgres']).withMessage('Invalid language'),
    handleValidationErrors
];
...
const validateObjectId = (paramName = 'id') => [
    param(paramName).custom((value) => {
        ...
        // In PostgreSQL/Prisma, IDs are UUIDs which also match the slug regex format
        // Allow MongoDB ObjectIds (24 hex), UUIDs, and generic alphanumeric slugs with hyphens/spaces/colons
