...
const contestService = {
    ...
    getAllContests: async (params = {}) => {
        try {
            const response = await apiClient.get('/contest/admin/all', { params });
            return response.data;
        } catch (error) {
            throw error.response?.data || { message: 'Failed to fetch contests' };
        }
    },
    // Start New Attempt (Solo Mode) → POST /contest/:contestId/start-attempt
    startNewAttempt: async (contestId) => {
        try {
            const response = await apiClient.post(`/contest/${contestId}/start-attempt`);
            return response.data;
        } catch (error) {
    ...
    // Get Contests by Course → GET /contest/course/:courseId
    getContestsByCourse: async (courseId) => {
