...
const problemService = {
    ...
    // Get All Content (Aggregated) → GET /api/content
    getAllContent: async () => {
        try {
    ...
    // If type is provided, hits the specific modular endpoint (e.g. /api/sql-problems/:id)
    // Otherwise falls back to the consolidated /api/content/:id resolver
    getProblemById: async (problemId, type = null) => {
        try {
            ...
            if (type) {
                ...
                const endpoint = typeMap[type.toLowerCase()];
                if (endpoint) url = `/${endpoint}/${problemId}`;
            } else {
                // Fallback for unified resolution
                url = `/content/${problemId}`;
            }
