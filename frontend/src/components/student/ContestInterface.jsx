...
/* —"—"—" Main Component —"—"—" */
const ContestInterface = ({ isPractice: isPracticeProp = false }) => {
    const { contestId } = useParams();
    const [searchParams] = useSearchParams();
    const isSoloParam = searchParams.get('mode') === 'solo';
    ...
    const isSolo = isSoloParam || contest?.isSolo;
    const courseIdParam = searchParams.get('courseId');
    const subIdParam = searchParams.get('subId');
    const isPracticeRequested = searchParams.get('isPractice') === 'true';
    const isPractice = isPracticeProp || isPracticeRequested;
    ...
    const COLLAPSED_SIDEBAR_WIDTH = 40; // px
    ...
    // Proctoring Hook - Disable in Practice Mode (unless Solo)
    const handleMaxViolations = useCallback(async (finalViolations) => {
        if ((isPractice && !isSolo) || autoSubmitTriggered.current || contestSubmitted) return;
        ...
        try {
            ...
            queryClient.invalidateQueries({ queryKey: ['contest_submissions'] });
            if (courseIdParam && subIdParam) {
                navigate(`/workspace/${courseIdParam}/${subIdParam}/contest/${contest?.slug || contestId}`, { replace: true });
            } else {
                setTimeout(() => {
    ...
    const { violations, isFullscreen, showViolationModal, currentViolationType, enterFullscreen, exitFullscreenSilently, markAsFinishing, getViolationSummary, resetViolations } =
        useProctoring(
            contestId,
    ...
    // Auto Fullscreen on mount if contest is active and NOT practice (unless Solo)
    useEffect(() => {
        if (!isPractice && contestActive && !contestSubmitted && !hasEnteredFullscreen && contest?.proctoringEnabled) {
            enterFullscreen();
            setHasEnteredFullscreen(true);
        }
    }, [contestActive, contestSubmitted, hasEnteredFullscreen, enterFullscreen, contest, isPractice, isSolo]);
    ...
    const handleStartNewAttempt = async () => {
        ...
        setFinishing(true);
        try {
            const res = await contestService.startNewAttempt(contestId);
            if (res.success) {
                toast.success('Starting a new attempt!');
                // Reset local state and reload data
                setContestSubmitted(false);
                setContestActive(true);
                setContestPast(false);
                setTimeRemaining(res.duration * 60); 
                
                // Crucial: Reset local frontend tracking for violations so it doesn't instantly auto-submit
                resetViolations();
                autoSubmitTriggered.current = false;
                
                // Refresh full data to get new start/end times & attempt number
                await queryClient.invalidateQueries({ queryKey: ['contest'] });
                await queryClient.invalidateQueries({ queryKey: ['contest_submissions'] });
                await fetchContest();
                // Invalidate any other queries related to this contest (like leaderboard or profile stats)
                queryClient.invalidateQueries(['contest']);
                
                // Clear code map for fresh attempt so problems are unlocked and completely fresh
                localStorage.removeItem(`contest_${contestId}_codeMap`);
                setUserCodeMap({});
            }
        } catch (error) {
    ...
    const handleContestEnd = useCallback(async () => {
        if (!contestSubmitted && (!isPractice || isSolo)) {
    ...
    const handleWebSocketMessage = useCallback((data) => {
        // ONLY allow executionResult in practice/solo mode (ignore global leaderboard/submissions)
        if ((isPractice || isSolo) && data.type !== 'executionResult') return; 
        switch (data.type) {
    ...
    }, [contestActive, contest, contestId, contestSubmitted, isPractice, isSolo]);
        ...
        try {
            const [contestData, submissionsData] = await Promise.all([
                queryClient.fetchQuery({
                    ...
                    queryFn: () => contestService.getContestById(contestId),
                    staleTime: 30 * 60 * 1000, // 30 mins cache
                }),
                queryClient.fetchQuery({
                    queryKey: ['contest_submissions', contestId],
                    queryFn: () => contestService.getStudentContestSubmissions(contestId),
                    staleTime: 5 * 60 * 1000, // 5 mins cache
                }).catch(() => ({ submissions: [] }))
            ]);
            setContest(contestData.contest);
            const isSub = isPractice ? false : (contestData.contest.isSubmitted || (isSolo && contestData.contest.isAttemptCompleted) || false);
            setContestSubmitted(isSub);
            ...
            if (isPractice) {
                ...
                if (contestData.contest.isSubmitted || (isSolo && contestData.contest.isAttemptCompleted)) {
                    // Contest already submitted — show past-contest view
                    // BUT: don't redirect if it's a Solo/Course contest so they can re-attempt or see results here
                    if (!isSolo && (contestData.contest.type === 'global' || !contestData.contest.batchId)) {
    ...
    useEffect(() => {
        if (!showLeaderboard) return; // Only fetch if board is explicitly open, no inline auto-fetch for solo anymore
        let cancelled = false;
        const fetchLb = async () => {
    ...
    if (!isPractice && (contestPast || (isSolo && contestSubmitted)) && contest && (!contestActive || (isSolo && contestSubmitted))) {
        ...
        return (
            <div className="min-h-screen bg-gray-50 dark:bg-[#111117] flex flex-col no-scrollbars-all">
                {/* Minimal header */}
                <header className="h-14 bg-[#F1F3F4] dark:bg-[#111117] border-b border-gray-100 dark:border-gray-800 flex items-center px-6 gap-4 shadow-sm transition-colors">
                    ...
                    <div className="card w-full max-w-3xl">
                        ...
                        <div className="flex items-start justify-between gap-4 mb-4">
                            ...
                            <div className="flex items-center gap-2 shrink-0">
                                {(isSolo && contest.isAttemptCompleted && (contest.maxAttempts === null || (contest.completedAttempts || contest.currentAttempt || 1) < (contest.maxAttempts || 99))) ? (
                                    <button 
                                        onClick={handleStartNewAttempt}
                                        disabled={finishing}
                                        className="btn btn-secondary flex items-center gap-1.5 bg-indigo-50 hover:bg-indigo-100 text-indigo-700 border-indigo-200 h-8 px-4 text-xs rounded-lg font-bold transition-all active:scale-95"
                                    >
                                        {finishing ? <Loader2 size={14} className="animate-spin" /> : <Plus size={14} />}
                                        Reattempt
                                    </button>
                                ) : isSolo && contest.isAttemptCompleted ? (
                                    <button
                                        onClick={() => {
                                            if (courseIdParam && subIdParam) {
                                                navigate(`/contests/${contest.slug || contestId}?mode=solo&isPractice=true&courseId=${courseIdParam}&subId=${subIdParam}`);
                                            } else {
                                                navigate(`/contests/${contest.slug || contestId}/practice`);
                                            }
                                        }}
                                        className="btn btn-secondary flex items-center gap-1.5 bg-emerald-50 hover:bg-emerald-100 text-emerald-700 border-emerald-200 h-8 px-4 text-xs rounded-lg font-bold transition-all active:scale-95"
                                    >
                                        <Code2 size={14} />
                                        Practice
                                    </button>
                        ...
                        {/* Stats row */}
                        <div className={`grid ${isSolo && contest.maxAttempts > 1 ? 'grid-cols-3' : 'grid-cols-2'} gap-4 py-4 border-t border-b border-gray-100 mb-4`}>
                            <div className="text-center border-r border-gray-100 last:border-0">
                                <div className="text-xs text-gray-500 uppercase font-medium mb-1 tracking-wider">Total Problems</div>
                                <div className="text-2xl font-black text-gray-900">{contest.problems?.length || 0}</div>
                            </div>
                            <div className="text-center border-gray-100 last:border-0 border-r">
                                <div className="text-xs text-gray-500 uppercase font-medium mb-1 tracking-wider">Duration</div>
                                <div className="text-xl font-bold text-gray-900 mt-1">{durationStr}</div>
                            </div>
                            {isSolo && contest.maxAttempts > 1 && (
                                <div className="text-center border-gray-100 last:border-0">
                                    <div className="text-xs text-gray-500 uppercase font-medium mb-1 tracking-wider">Attempts Used</div>
                                    <div className="text-xl font-bold text-gray-900 mt-1">
                                        <span className="text-indigo-600">{Math.min(contest.completedAttempts || contest.currentAttempt || 1, contest.maxAttempts)}</span>
                                        <span className="text-gray-400 mx-1">/</span>
                                        <span>{contest.maxAttempts}</span>
                                    </div>
                                </div>
                            )}
                        </div>

                        {/* Score and Reattempt CTA for Solo Mode */}
                        {isSolo && contest.isAttemptCompleted && (
                            <div className="mt-4 space-y-4">
                                {/* Score Display */}
                                <div className="p-6 bg-emerald-50 dark:bg-emerald-900/10 rounded-2xl border border-emerald-100 dark:border-emerald-800/50 flex items-center justify-between">
                                    <div className="flex items-center gap-4">
                                        <div className="w-12 h-12 rounded-full bg-emerald-100 dark:bg-emerald-900/30 flex items-center justify-center text-emerald-600 dark:text-emerald-400">
                                            <Trophy size={24} />
                                        </div>
                                        <div>
                                            <h3 className="text-lg font-extrabold text-emerald-900 dark:text-emerald-100">Best Overall Score</h3>
                                            <p className="text-sm text-emerald-600 dark:text-emerald-400 font-medium whitespace-pre-wrap leading-relaxed">
                                                Score: <span className="text-emerald-700 dark:text-emerald-300 font-black">{contest.lastAttemptResults?.score || 0}</span> — High score across all attempts
                                            </p>
                                        </div>
                                    </div>
                                    <div className="text-right">
                                        <div className="text-2xl font-black text-emerald-600">{contest.lastAttemptResults?.problemsSolved || 0}</div>
                                        <div className="text-[10px] uppercase font-bold text-emerald-500 opacity-70">Solved</div>
                                    </div>
                                </div>

                                {/* Reattempt Option moved to header for better visibility */}
                            </div>
                        )}
                    </div>

                    {/* Inline Leaderboard for Solo Contests - Removed per user request */}
    ...
    return (
        <div className="flex flex-col h-screen bg-gray-50 dark:bg-[#111117] font-sans text-gray-800 dark:text-gray-200 overflow-hidden relative transition-colors no-scrollbars-all" ref={containerRef}>
            ...
            {/* —"—"—" Minimal Header —"—"—" */}
            <header className="h-14 bg-[#F1F3F4] dark:bg-[#111117] border-b border-gray-100 dark:border-gray-800 flex items-center justify-between px-4 z-20 shrink-0 shadow-sm dark:shadow-black/20 relative transition-colors">
                ...
                <div className="flex items-center gap-3">
                    ...
                    */}
                    {/* Attempt Counter (Solo Only) */}
                    {isSolo && contest?.maxAttempts > 1 && (
                        <div className="flex items-center gap-2 px-3 py-1.5 bg-indigo-50 dark:bg-indigo-900/10 text-indigo-600 dark:text-indigo-400 border border-indigo-100 dark:border-indigo-800 rounded-lg text-[11px] font-bold shadow-sm shrink-0">
                            <Layers size={13} className="text-indigo-500" />
                            <span className="tracking-tight text-indigo-600 dark:text-indigo-400">
                                <span className="font-extrabold">{Math.min(contest.completedAttempts || contest.currentAttempt || 1, contest.maxAttempts)}</span>
                                <span className="opacity-40 mx-0.5">/</span>
                                <span className="font-extrabold">{contest.maxAttempts}</span>
                            </span>
                        </div>
                    )}
                    ...
                    {!isPractice && contest?.proctoringEnabled && (
                        <div className="flex items-center gap-3 text-xs text-gray-500 border-r border-gray-200 dark:border-gray-800 pr-4 mr-1">
                    ...
                    {!isPractice && !contestSubmitted && (!isPractice || isSolo) && timeRemaining !== null && (
                        <button
                    ...
                    {(isPractice || isSolo) && (
                        <div className="flex items-center gap-3">
                            ...
                            <button
                                onClick={() => {
                                    if (courseIdParam && subIdParam) {
                                        navigate(`/workspace/${courseIdParam}/${subIdParam}/contest/${contest?.slug || contestId}`);
                                    } else {
            ...
            <div className={`flex-1 flex flex-col lg:flex-row relative transition-colors ${isMobile ? 'overflow-y-auto overflow-x-hidden' : 'overflow-hidden'}`}>
                ...
                >
                    <div className="flex-1 overflow-hidden flex flex-col relative h-full">
                        ...
                        <div className={`flex-1 flex flex-col overflow-hidden h-full transition-all duration-300 ${!showSidebar ? 'hidden' : 'flex'}`}>
                            {showSidebar && (
                                <div className="px-5 py-4 border-b border-gray-100 dark:border-gray-800 bg-gray-50 dark:bg-[#111117] shrink-0 flex items-center justify-between transition-colors">
                            ...
                            {/* Problem List */}
                            <div className={`flex-1 overflow-y-auto custom-thin-scrollbar ${showSidebar ? 'p-3' : 'p-1.5'} space-y-1 bg-gray-50 dark:bg-[#111117] transition-colors`}>
                                {contest?.problems?.map((p, i) => {
                                    ...
                                    return (
                                        <button
                                            ...
                                            disabled={isLocked}
                                            title={!showSidebar ? p.title : ''}
                                            className={`w-full text-left rounded-xl transition-all border flex gap-3 ${showSidebar ? 'p-3' : 'p-2 justify-center'} ${isLocked
                                                ? 'bg-gray-50 dark:bg-[#111117] border-[var(--color-border-interactive)] opacity-60 cursor-not-allowed pointer-events-none'
                                                : isActive
                                                    ? (isSolved ? 'bg-emerald-50 dark:bg-emerald-900/20 border-emerald-200 dark:border-emerald-800 shadow-sm' : 'bg-[#F1F3F4] dark:bg-[#111117] border-purple-200 dark:border-purple-800 shadow-sm ring-1 ring-purple-500/20')
                                                    : isSolved
                                                        ? 'bg-[#F1F3F4] dark:bg-[#111117] border-[var(--color-border-interactive)] hover:border-emerald-300 dark:hover:border-emerald-700'
                                                        : 'bg-[#F1F3F4] dark:bg-[#111117] border-transparent dark:border-gray-700 hover:bg-gray-100 dark:hover:bg-[#23232e]'
                                                }`}
                                        >
                                            <div className={`w-7 h-7 sm:w-8 sm:h-8 rounded-lg shrink-0 flex items-center justify-center font-bold text-xs sm:text-sm shadow-sm border ${isActive
                                                ...
                                                : isSolved
                                                    ...
                                                    : 'bg-gray-100 dark:bg-[#111117] border-[var(--color-border-interactive)] text-gray-500 dark:text-gray-400'
                                                }`}>
                    ...
                    >
                        <div className="flex flex-col items-center justify-center gap-0.5 opacity-60 group-hover:opacity-100">
                            {showSidebar ? <ChevronLeft size={10} strokeWidth={3} /> : <ChevronRight size={10} strokeWidth={3} />}
                        </div>
                    </button>
                </div>
                ...
                <div style={isMobile ? {
                    ...
                    width: `${descW}%`,
                    transition: isResizing ? 'none' : 'width 0.3s cubic-bezier(0.2, 0.8, 0.2, 1)'
                }} className="flex flex-col overflow-hidden shrink-0 border-r border-gray-200 dark:border-gray-800 border-b md:border-b-0 bg-[#F1F3F4] dark:bg-[#111117] transition-colors relative">

                    <div className="flex items-center h-12 border-b border-gray-100 dark:border-gray-800 bg-[#F1F3F4] dark:bg-[#111117] shrink-0 pl-0 pr-4 overflow-x-auto no-scrollbar transition-colors">
                        <div className="flex items-center gap-1 h-full">
                            <button
                                onClick={() => setShowEditorial(false)}
                                className={`h-full flex items-center gap-2 px-4 text-xs font-bold border-b-2 transition-colors whitespace-nowrap ${!showEditorial ? 'border-purple-500 text-purple-700 dark:text-purple-400 bg-[#F1F3F4] dark:bg-[#111117]' : 'border-transparent text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-200 hover:bg-gray-50 dark:hover:bg-[#23232e]'}`}
                            >
                                ...
                                <div className="space-y-6">
                                    ...
                                    {selectedProblem.constraints?.length > 0 && (
                                        <div>
                                            ...
                                            <ul className="bg-gray-50 dark:bg-[#111117] border border-gray-100 dark:border-gray-800 rounded-lg px-4 py-2.5 space-y-1">
                                    ...
                                    {(selectedProblem.inputFormat || extractInputFormat(selectedProblem.description)) && (
                                        <div>
                                            ...
                                            <div className="bg-gray-50 dark:bg-[#111117] border border-gray-100 dark:border-gray-800 rounded-lg px-4 py-2.5 text-sm text-gray-700 dark:text-gray-300 prose prose-sm max-w-none [&>p:last-child]:!mb-0">
                                                <ReactMarkdown remarkPlugins={[remarkGfm, remarkBreaks]} components={MarkdownComponents}>
                                                    {selectedProblem.inputFormat || extractInputFormat(selectedProblem.description)}
                                                </ReactMarkdown>
                                            </div>
                                        ...
                                        <div key={i} className="rounded-lg border border-gray-100 dark:border-gray-800 overflow-hidden">
                                            ...
                                            <div className="p-4 space-y-3 bg-[#F1F3F4] dark:bg-[#111117]">
                                                <div>
                                                    ...
                                                    <pre className="bg-gray-50 dark:bg-[#111117] border border-gray-100 dark:border-gray-800 rounded p-2 text-xs font-mono text-gray-800 dark:text-gray-200 whitespace-pre-wrap">{ex.input}</pre>
                                                </div>
                                                <div>
                                                    <p className="text-[10px] font-bold text-gray-400 dark:text-gray-500 uppercase mb-1">Output</p>
                                                    <pre className="bg-gray-50 dark:bg-[#111117] border border-gray-100 dark:border-gray-800 rounded p-2 text-xs font-mono text-gray-800 dark:text-gray-200 whitespace-pre-wrap">{ex.output}</pre>
                                                </div>
                                                {ex.explanation && (
                                                    <div>
                                    ...
                                    {selectedProblem.edgeCases?.length > 0 && (
                                        <div>
                                            ...
                                            <ul className="bg-gray-50 dark:bg-[#111117] border border-gray-100 dark:border-gray-800 rounded-lg p-4 space-y-1">
                                    ...
                                    {(selectedProblem.timeComplexity || selectedProblem.spaceComplexity) && (
                                        <div>
                                            ...
                                            <div className="bg-gray-50 dark:bg-[#111117] border border-gray-100 dark:border-gray-800 rounded-lg p-4 space-y-2">
                                                {selectedProblem.timeComplexity && (
                                                    <div className="flex items-center gap-2">
                                                        ...
                                                        <span className="text-sm font-mono text-gray-800 dark:text-gray-200 bg-[#F1F3F4] dark:bg-[#111117] border border-gray-100 dark:border-gray-800 px-2 py-0.5 rounded shadow-sm">{selectedProblem.timeComplexity}</span>
                                                    </div>
                                                )}
                                                {selectedProblem.spaceComplexity && (
                                                    <div className="flex items-center gap-2">
                                                        <span className="text-xs font-bold text-gray-500 dark:text-gray-400 uppercase">Space:</span>
                                                        <span className="text-sm font-mono text-gray-800 dark:text-gray-200 bg-[#F1F3F4] dark:bg-[#111117] border border-gray-100 dark:border-gray-800 px-2 py-0.5 rounded shadow-sm">{selectedProblem.spaceComplexity}</span>
                                                    </div>
                                                )}
                                            </div>
                                        </div>
                ...
                <div style={isMobile ? {
                        ...
                        <div className="flex-1 overflow-hidden relative">
                            ...
                            {(isProblemLocked || contestSubmitted) && (
                                <div className="absolute inset-0 bg-gray-50/50 dark:bg-[#111117] backdrop-blur-[1px] flex items-center justify-center z-10 pointer-events-none">
                                    <div className="bg-[#F1F3F4] dark:bg-[#111117] p-6 rounded-2xl shadow-xl border border-gray-100 dark:border-gray-700 text-center transform scale-100">
                    ...
                    <div
                        ...
                        className="flex flex-col bg-[#F1F3F4] dark:bg-[#111117] overflow-hidden relative"
                        data-results-panel
                    >
                        <div className="flex items-center h-10 border-b border-gray-100 dark:border-gray-800 bg-[#F1F3F4] dark:bg-[#111117] px-2 gap-1 shrink-0">
                            <button onClick={() => setBottomTab('testcases')}
                                className={`px-4 h-full text-xs font-medium border-b-2 transition-colors flex items-center gap-2 ${bottomTab === 'testcases' ? 'border-purple-500 text-purple-700 dark:text-purple-400 bg-[#F1F3F4] dark:bg-[#111117]' : 'border-transparent text-gray-500 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-[#23232e] hover:text-gray-700 dark:hover:text-gray-200'}`}>
                                <List size={14} /> Test Cases
                            </button>
                            <button onClick={() => setBottomTab('results')}
                                className={`px-4 h-full text-xs font-medium border-b-2 transition-colors flex items-center gap-2 ${bottomTab === 'results' ? 'border-purple-500 text-purple-700 dark:text-purple-400 bg-[#F1F3F4] dark:bg-[#111117]' : 'border-transparent text-gray-500 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-[#23232e] hover:text-gray-700 dark:hover:text-gray-200'}`}>
                                {isCompileErr ? (
                        ...
                        <div className="flex-1 overflow-hidden bg-[#F1F3F4] dark:bg-[#111117] relative group/results transition-colors">
                                ...
                                <>
                                    ...
                                    {bottomTab === 'testcases' && (
                                        <div className="flex flex-col h-full font-problem bg-[#F1F3F4] dark:bg-[#111117] transition-colors">
                                            ...
                                            <div className="flex items-center gap-0.5 px-2 py-1.5 border-b border-gray-100 dark:border-gray-700 overflow-x-auto scrollbar-hide shrink-0 bg-[#F1F3F4] dark:bg-[#111117]">
                                                        ...
                                                        return (
                                                            <div className="space-y-4 w-full px-1">
                                                                <div>
                                                                    ...
                                                                    <div className="w-full bg-gray-50/50 dark:bg-[#111117] border border-gray-100 dark:border-gray-800 rounded-lg p-3 text-sm font-mono text-gray-800 dark:text-gray-200 whitespace-pre-wrap select-text transition-colors">
                                                                        {tc.input}
                                                                    </div>
                                                                </div>
                                                                <div>
                                                                    <p className="text-[10px] font-bold text-gray-400 dark:text-gray-500 uppercase mb-1.5">Expected Output</p>
                                                                    <div className="w-full bg-gray-50/50 dark:bg-[#111117] border border-gray-100 dark:border-gray-800 rounded-lg p-3 text-sm font-mono text-gray-600 dark:text-white whitespace-pre-wrap select-text transition-colors">
                                                                        {tc.output}
                                                                    </div>
                                                                </div>
                                                            </div>
                                                    ...
                                                    (() => {
                                                        ...
                                                        return (
                                                            <div className="space-y-2 h-full flex flex-col w-full px-1">
                                                                ...
                                                                <textarea
                                                                    className="flex-1 w-full bg-gray-50 dark:bg-[#111117] border border-gray-100 dark:border-gray-800 rounded-lg p-3 text-sm font-mono text-gray-800 dark:text-gray-200 focus:ring-1 focus:ring-primary-400 focus:border-primary-400 outline-none resize-none min-h-[100px] transition-colors"
                                                                    value={cCase.input}
                                    ...
                                    {/* Results Tab */}
                                    {bottomTab === 'results' && (
                                        <div className="h-full overflow-y-auto flex flex-col bg-[#F1F3F4] dark:bg-[#111117] transition-colors" style={{ animation: 'slide-up-results 0.28s cubic-bezier(0.16,1,0.3,1) both' }}>
                                                ...
                                                <div className="flex flex-col bg-red-50/30 dark:bg-red-900/10">
                                                    <div className="bg-red-50 dark:bg-red-900/20 border-b border-red-100 dark:border-red-900/30 px-4 py-3 flex items-center gap-2 shrink-0">
                                                        ...
                                                        <button
                                                            ...
                                                            className="px-3 py-1 bg-[#F1F3F4] dark:bg-[#111117] border border-red-200 dark:border-red-800 text-red-600 dark:text-red-400 rounded-md text-[10px] font-bold hover:bg-red-50 dark:hover:bg-red-900/20 transition-colors"
                                                        >
                                                            Retry
                                                        </button>
                                                    </div>
                                            ...
                                            {!running && !submitting && !isCompileErr && displayResult && (
                                                <div className="flex flex-col h-full font-problem">
                                                    ...
                                                    {displayResult.isRun && displayResult.results?.length > 0 && (
                                                        <>
                                                            <div className="flex items-center gap-1.5 px-4 py-2 border-b border-gray-100 dark:border-gray-700 overflow-x-auto scrollbar-hide shrink-0 bg-[#F1F3F4] dark:bg-[#111117]">
                                                                ...
                                                                {displayResult.results[activeResultCase] ? (
                                                                    <div className="space-y-4 w-full px-1">
                                                                        ...
                                                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                                                            ...
                                                                            <div>
                                                                                <p className="text-[10px] font-bold text-gray-400 dark:text-gray-500 uppercase mb-1.5">Expected Output</p>
                                                                                <div className="bg-gray-50/50 dark:bg-[#111117] border border-gray-100 dark:border-gray-800 rounded-lg p-3 text-sm font-mono text-gray-600 dark:text-white whitespace-pre-wrap min-h-[48px] transition-colors">
            ...
            {!isOnline && (
                <div className="fixed inset-0 z-[999999] flex items-center justify-center bg-[#111117]/90 backdrop-blur-sm">
                    <div className="bg-[#F1F3F4] dark:bg-[#111117] p-8 rounded-2xl shadow-2xl max-w-sm w-full text-center border border-gray-100 dark:border-gray-700">
            ...
            {showFinishModal && (
                <div className="fixed inset-0 z-[99999] flex items-center justify-center bg-black/60 dark:bg-[#111117]/80 backdrop-blur-sm animate-in fade-in zoom-in duration-200">
                    <div className="bg-[#F1F3F4] dark:bg-[#111827] rounded-2xl shadow-2xl p-8 max-w-sm w-full text-center border border-gray-100 dark:border-gray-800 transform scale-100 transition-colors">
            ...
            {(!isPractice && contestActive && !contestSubmitted && contest?.proctoringEnabled && !isFullscreen && !finishing && isOnline) && (
                <div className="fixed inset-0 z-[99998] flex flex-col items-center justify-center bg-[#111117]/95 backdrop-blur-md">
                    <div className="bg-[#F1F3F4] dark:bg-[#111117] p-8 rounded-3xl shadow-2xl max-w-sm w-full text-center border border-gray-100 dark:border-gray-700 transform scale-100 transition-transform">
            ...
            {showViolationModal && (
                <div className="fixed inset-0 z-[99999] flex items-center justify-center bg-red-900/60 backdrop-blur-sm animate-in fade-in zoom-in duration-200">
                    <div className="bg-[#F1F3F4] dark:bg-[#111117] rounded-2xl shadow-2xl p-8 max-w-sm w-full text-center border-4 border-red-500 transform scale-100 transition-colors">
                        ...
                        <div className="bg-gray-50 dark:bg-[#111117] rounded-xl p-4 border border-gray-100 dark:border-gray-800">
            ...
            {showLeaderboard && (
                <div className="fixed inset-0 z-[1000] flex items-center justify-center bg-[#111117]/50 backdrop-blur-sm p-4 sm:p-6 animate-in fade-in duration-200">
                    <div className="bg-[#F1F3F4] dark:bg-[#111117] rounded-2xl shadow-2xl w-full max-w-5xl h-[80vh] flex flex-col overflow-hidden border border-gray-100 dark:border-gray-800" onClick={e => e.stopPropagation()}>
                        <div className="bg-[#F1F3F4] dark:bg-[#111117] border-b border-gray-100 dark:border-gray-700 p-5 flex justify-between items-start shrink-0">
                            <div className="flex-1 min-w-0">
                        ...
                        <div className="px-5 py-3 border-b border-gray-100 dark:border-gray-700 flex items-center gap-3 bg-gray-50 dark:bg-[#111117] shrink-0">
                            ...
                            {[20, 50, 100, 200, 500].map(size => (
                                <button key={size} onClick={() => { setItemsPerPage(size); setCurrentPage(1); }}
                                    className={`px-2.5 py-1 rounded-md text-xs font-semibold border transition-colors ${itemsPerPage === size ? 'bg-purple-600 text-white border-purple-600' : 'bg-[#F1F3F4] dark:bg-[#111117] text-gray-600 dark:text-gray-400 border-gray-300 dark:border-gray-700 hover:border-purple-400 hover:text-purple-600'
                                        }`}>{size}</button>
                            ))}
                        ...
                        <div className="flex-1 overflow-auto scrollbar-thin bg-gray-50/50 dark:bg-[#111117] relative">
                            <div className="overflow-auto">
                                <table className="w-full text-left border-separate border-spacing-0 min-w-max">
                                    <thead className="bg-[#F1F3F4] dark:bg-[#111117] sticky top-0 z-[100] shadow-sm text-xs uppercase tracking-wider text-gray-900 dark:text-white font-bold">
                                        <tr>
                                            <th className="px-3 py-4 text-center text-xs font-bold text-gray-900 dark:text-white uppercase tracking-wider sticky left-0 bg-[#F1F3F4] dark:bg-[#111117] z-[110] border-b border-gray-100 dark:border-gray-800 border-r border-gray-200 dark:border-gray-800 min-w-[60px]">
                                                Rank
                                            </th>
                                            <th className="px-3 py-4 text-left text-xs font-bold text-gray-900 dark:text-white uppercase tracking-wider sticky left-[60px] bg-[#F1F3F4] dark:bg-[#111117] z-[110] border-b border-gray-100 dark:border-gray-800 border-r border-gray-200 dark:border-gray-800 min-w-[110px]">
                                                Roll No
                                            </th>
                                            <th className="px-3 py-4 text-left text-xs font-bold text-gray-900 dark:text-white uppercase tracking-wider sticky left-[170px] bg-[#F1F3F4] dark:bg-[#111117] z-[110] border-b border-gray-100 dark:border-gray-800 border-r border-gray-200 dark:border-gray-800 min-w-[140px]">
                                                Full Name
                                            </th>
                                            <th className="px-3 py-3 text-left text-xs font-bold text-gray-900 dark:text-white uppercase cursor-pointer hover:bg-gray-100 dark:hover:bg-[#23232e] min-w-[120px] border-b border-gray-100 dark:border-gray-800 border-r border-gray-200 dark:border-gray-800" onClick={() => handleSort('username')}>
                                                Username {sortConfig.key === 'username' && (sortConfig.direction === 'asc' ? '▲' : '▼')}
                                            </th>
                                            <th className="px-3 py-3 text-left text-xs font-bold text-gray-900 dark:text-white uppercase cursor-pointer hover:bg-gray-100 dark:hover:bg-[#23232e] min-w-[120px] border-b border-gray-100 dark:border-gray-800 border-r border-gray-200 dark:border-gray-800" onClick={() => handleSort('branch')}>
                                                Branch {sortConfig.key === 'branch' && (sortConfig.direction === 'asc' ? '▲' : '▼')}
                                            </th>

                                            {/* Problem Columns — in contest order */}
                                            {contest?.problems?.map((prob, i) => (
                                                <th key={prob._id} className="p-4 font-bold text-center whitespace-nowrap min-w-[130px] border-b border-gray-100 dark:border-gray-800 border-r border-gray-50 dark:border-gray-800">
                                                    P{i + 1}: {prob.title?.length > 12 ? prob.title.slice(0, 12) + '—' : prob.title}
                                                </th>
                                            ))}

                                            <th className="p-4 font-bold text-center cursor-pointer hover:bg-gray-50 dark:hover:bg-[#23232e] whitespace-nowrap border-b border-gray-100 dark:border-gray-800 border-r border-gray-200 dark:border-gray-800" onClick={() => handleSort('time')}>
                                                Time {sortConfig.key === 'time' && (sortConfig.direction === 'asc' ? '▲' : '▼')}
                                            </th>
                                            <th className="p-4 font-bold text-center cursor-pointer hover:bg-gray-50 dark:hover:bg-[#23232e] whitespace-nowrap border-b border-gray-100 dark:border-gray-800 border-r border-gray-200 dark:border-gray-800" onClick={() => handleSort('problemsSolved')}>
                                                Solved {sortConfig.key === 'problemsSolved' && (sortConfig.direction === 'asc' ? '▲' : '▼')}
                                            </th>
                                            {contest?.proctoringEnabled && (
                                                <>
                                                    <th className="p-4 font-bold text-center whitespace-nowrap border-b border-gray-100 dark:border-gray-800 border-r border-gray-200 dark:border-gray-800">Tab Switches</th>
                                                    <th className="p-4 font-bold text-center whitespace-nowrap border-b border-gray-100 dark:border-gray-800 border-r border-gray-200 dark:border-gray-800">FS Exits</th>
                                                    <th className="p-4 font-bold text-center whitespace-nowrap text-amber-600 border-b border-gray-100 dark:border-gray-800 border-r border-gray-200 dark:border-gray-800">—  Violations</th>
                                                </>
                                            )}
                                            <th className="p-4 font-bold text-center cursor-pointer hover:bg-gray-50 dark:hover:bg-[#23232e] whitespace-nowrap border-b border-gray-100 dark:border-gray-800 border-r border-gray-200 dark:border-gray-800" onClick={() => handleSort('status')}>
                                                Status {sortConfig.key === 'status' && (sortConfig.direction === 'asc' ? '▲' : '▼')}
                                            </th>
                                            <th className="px-3 py-4 text-center text-xs font-bold text-gray-900 dark:text-white uppercase tracking-wider sticky right-0 bg-[#F1F3F4] dark:bg-[#111117] z-[110] border-l border-[var(--color-border-interactive)] border-b border-gray-100 dark:border-gray-800 min-w-[100px]">
                                                Total Coins
                                            </th>
                                        </tr>
                                    </thead>
                                    {!loadingLeaderboard && (
                                        <tbody className="text-sm bg-[#F1F3F4] dark:bg-[#111117]">
                                            {(() => {
                                                ...
                                                return (
                                                    <>
                                                        ...
                                                        {paginatedData.map((entry, pageIndex) => {
                                                            ...
                                                            return (
                                                                <tr key={index} className="group hover:bg-gray-50 dark:hover:bg-[#23232e] transition bg-[#F1F3F4] dark:bg-[#111117]">
                                                                    <td className="px-3 py-3 whitespace-nowrap sticky left-0 bg-[#F1F3F4] dark:bg-[#111117] z-40 border-b border-gray-100 dark:border-gray-700 border-r border-gray-100 dark:border-gray-700 group-hover:bg-gray-50 dark:group-hover:bg-[#23232e]">
                                                                        <div className={`w-8 h-8 rounded-full flex items-center justify-center mx-auto font-bold shadow-sm ${rowRank === 1 ? 'bg-yellow-100 border-2 border-yellow-300 text-xl' :
                                                                            rowRank === 2 ? 'bg-gray-100 border-2 border-gray-300 text-xl' :
                                                                                rowRank === 3 ? 'bg-orange-100 border-2 border-orange-300 text-xl' : 'bg-[#F1F3F4] dark:bg-[#111117] border border-gray-100 dark:border-gray-800 text-gray-500 dark:text-gray-400 text-base'
                                                                            }`}>
                                                                            {rowRank === 1 ? '🥇' : rowRank === 2 ? '🥈' : rowRank === 3 ? '🥉' : `#${rowRank}`}
                                                                        </div>
                                                                    </td>
                                                                    {/* Roll No - sticky */}
                                                                    <td className="px-3 py-3 whitespace-nowrap text-sm text-gray-700 dark:text-gray-300 font-mono sticky left-[60px] bg-[#F1F3F4] dark:bg-[#111117] z-40 border-b border-gray-100 dark:border-gray-700 border-r border-gray-100 dark:border-gray-700 group-hover:bg-gray-50 dark:group-hover:bg-[#23232e]">{entry.rollNumber}</td>
                                                                    {/* Full Name - sticky */}
                                                                    <td className="px-3 py-3 text-sm text-gray-900 dark:text-gray-100 font-bold max-w-[140px] min-w-[140px] truncate sticky left-[170px] bg-[#F1F3F4] dark:bg-[#111117] z-40 border-b border-gray-100 dark:border-gray-700 border-r border-gray-100 dark:border-gray-700 overflow-hidden group-hover:bg-gray-50 dark:group-hover:bg-[#23232e]" title={entry.fullName !== 'N/A' ? entry.fullName : entry.username}>
                                                                        {entry.fullName !== 'N/A' ? entry.fullName : entry.username}
                                                                    </td>
                                                                    {/* Username */}
                                                                    <td className="px-3 py-3 text-sm text-gray-500 dark:text-gray-400 max-w-[120px] truncate border-b border-gray-100 dark:border-gray-700 bg-[#F1F3F4] dark:bg-[#111117] group-hover:bg-gray-50 dark:group-hover:bg-[#23232e]" title={(entry.isSpotUser || entry.username?.startsWith('spot_')) ? '' : entry.username}>
                                                                    ...
                                                                    {/* Branch */}
                                                                    <td className="px-3 py-3 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400 border-b border-gray-100 dark:border-gray-700 bg-[#F1F3F4] dark:bg-[#111117] group-hover:bg-gray-50 dark:group-hover:bg-[#23232e]">{entry.branch}</td>
                                                                    ...
                                                                    {contest?.problems?.map(prob => {
                                                                        ...
                                                                        return (
                                                                            <td key={prob._id} className="p-2 text-center border-b border-gray-100 dark:border-gray-700 border-r border-gray-50 dark:border-gray-800 bg-[#F1F3F4] dark:bg-[#111117] group-hover:bg-gray-50 dark:group-hover:bg-[#23232e]">
                                                                                <div className="flex flex-col items-center justify-center gap-0.5">
                                                                                    <div className={`px-2 py-1 rounded text-xs inline-flex items-center gap-1 min-w-[72px] justify-center ${cellClass}`}>
                                                                                        {icon && <span className="font-bold">{icon}</span>}
                                                                                        {status === 'Not Attempted' ? '—' : status === 'Accepted' ? 'Accepted' : 'Wrong'}
                                                                    ...
                                                                    <td className="px-3 py-2 whitespace-nowrap text-sm text-center text-gray-600 font-mono border-b border-gray-100 dark:border-gray-700 bg-[#F1F3F4] dark:bg-[#111117] group-hover:bg-gray-50 dark:group-hover:bg-[#23232e]">
                                                                        {(() => {
                                                                    ...
                                                                    <td className="px-3 py-2 whitespace-nowrap text-center border-b border-gray-100 dark:border-gray-700 bg-[#F1F3F4] dark:bg-[#111117] group-hover:bg-gray-50 dark:group-hover:bg-[#23232e]">
                                                                    ...
                                                                    {contest?.proctoringEnabled && (() => {
                                                                        ...
                                                                        return (
                                                                            <>
                                                                                <td className="px-3 py-2 whitespace-nowrap text-sm text-center border-b border-gray-100 dark:border-gray-700 bg-[#F1F3F4] dark:bg-[#111117] group-hover:bg-gray-50 dark:group-hover:bg-[#23232e]">
                                                                    ...
                                                                    <td className="p-4 text-center border-b border-gray-100 dark:border-gray-700 bg-[#F1F3F4] dark:bg-[#111117] group-hover:bg-gray-50 dark:group-hover:bg-[#23232e]">
                                                                        {entry.isCompleted ? (
                                                                    ...
                                                                    <td className="p-4 text-center font-bold text-purple-600 dark:text-purple-400 sticky right-0 bg-[#F1F3F4] dark:bg-[#111117] z-[50] border-b border-gray-100 dark:border-gray-700 border-l border-gray-100 dark:border-gray-700 group-hover:bg-gray-50 dark:group-hover:bg-[#23232e]">{entry.score}</td>
                                                                </tr>
                                                            )
                                                        })}
                                                    </>
                        ...
                        {totalPages > 1 && !loadingLeaderboard && leaderboardData.length > 0 && (
                            <div className="bg-[#F1F3F4] dark:bg-[#111117] border-t border-gray-100 dark:border-gray-700 p-4 flex items-center justify-between mt-auto shrink-0 w-full z-20 shadow-[0_-4px_6px_-4px_rgba(0,0,0,0.05)]">
