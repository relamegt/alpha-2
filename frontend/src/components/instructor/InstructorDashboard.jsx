...
const InstructorDashboard = () => {
    ...
    const { data: dashboardData, isLoading } = useQuery({
        ...
        queryFn: async () => {
            const [batchData, submissionsData] = await Promise.all([
                adminService.getAllBatches(),
                submissionService.getRecentSubmissions(10)
            ]);
    ...
    const stats = dashboardData?.stats || { batches: [], totalStudents: 0, activeBatchesCount: 0 };
                            ...
                            <table className="w-full text-left">
                                ...
                                <tbody className="divide-y divide-gray-100 dark:divide-gray-700">
                                    {recentSubmissions.map((sub) => {
                                        const verdict = sub.status || sub.verdict;
                                        const problemType = (sub.problem?.type || 'problem').toLowerCase();
                                            ...
                                            <tr key={sub.id || sub._id} className="hover:bg-gray-50/50 dark:hover:bg-gray-700/30 transition-colors">
                                                ...
                                                <td className="px-6 py-4">
                                                    ...
                                                    <div className="text-xs text-gray-500 dark:text-gray-400">
                                                        {isCodingProblem ? sub.language : '-'}
                                                    </div>
                                                </td>
                                                <td className="px-6 py-4">
                                                    {isCodingProblem ? (
                                                        <span className={`px-2.5 py-1 rounded-full text-xs font-medium border ${getStatusColor(verdict)}`}>
                                                            {verdict}
                                                        </span>
                                                    ) : (
