 ...
 */
const useProctoring = (contestId, studentId, isActive, onMaxViolations, maxViolations = 5, attemptNumber = 1) => {
    ...
    useEffect(() => {
        if (contestId && studentId) {
            lsKeyRef.current = `proctoring_${contestId}_${studentId}_${attemptNumber}`;
        }
    }, [contestId, studentId, attemptNumber]);
    ...
    const [violations, setViolations] = useState(() => {
        if (contestId && studentId) {
            try {
                const cached = JSON.parse(localStorage.getItem(`proctoring_${contestId}_${studentId}_${attemptNumber}`));
    ...
    const lastAttemptRef = useRef(attemptNumber);
    useEffect(() => {
        if (!contestId || !studentId) return;
        
        // If attempt number changed, we must resync or reset
        if (lastAttemptRef.current !== attemptNumber) {
            hasSyncedFromLS.current = false;
            lastAttemptRef.current = attemptNumber;
        }

        if (hasSyncedFromLS.current) return; // only do this once per attempt
        
        const key = `proctoring_${contestId}_${studentId}_${attemptNumber}`;
        ...
        try {
            ...
            if (cached) {
                ...
                // If no cache for this new attempt, reset violations to zero
